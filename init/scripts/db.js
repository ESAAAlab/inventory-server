...
module.exports = {
  ...
  sync: function () {
    ...
    models.sequelize.sync({
      ...
      function () {
        fixtures.loadFile('scripts/assets/init.json', models).then(function () {})
      }
    )
  },

  randomInventory: function (nb) {
        ...
        for (var i in data.results) {
          ...
          models.user.create({
            ...
            throttledRequest({
              ...
              console.log('downloading thumbnail for user ' + index + '/' + nb + ' : ' + sqluser.firstName + ' ' + sqluser.lastName)
              if (!error && response.statusCode === 200) {
                ...
                console.log('userProfilePic added for user ' + index + '/' + nb + ' : ' + sqluser.firstName + ' ' + sqluser.lastName)
              } else {
                console.error('error downloading picture for user ' + index + '/' + nb + ' : ' + sqluser.firstName + ' ' + sqluser.lastName)
                console.error(error)
              }
            })
          })
